...
def check_name(n, name_2):
    ...
    if name_2 == 'absolute path to file':
        ...
        if next_name.upper() in param.library_list:
...
def check_index(
    ...
    for index_name in index_name_list:
        if index_name in param.index_dict:
            if (index_name in param.mapping_indices and rep_type == 'mapping') or (index_name in param.norm_or_expr_indices and rep_type == 'expression') or (index_name in param.norm_or_expr_indices and rep_type == 'normalization'):
    ...
    if valid_indices:
        libr_dictionary[libr_name][rep_type][name_file].append(valid_indices)
    ...
    for file in file_diction:
        ...
        for _ in range(lib_count):
            ...
            if library_name != 'H':
                ...
                print('Enter indices for hybrid test in file %s.\nIf there are more then one index per replicate, print indices in one line separated by spaces. Finally press N.'
                        % file
                        )
                if library_name not in library_dict:
                    library_dict['H'] = {file: []}
                ...
                while ind != 'N':
                    if ind in param.mapping_indices:
            ...
            for rep_type in library_dict[library]:
                for file in library_dict[library][rep_type]:
                    for replicate in library_dict[library][rep_type][file]:
                        print('{0} replicate with indices {1} in file {2}.'.format(
                                rep_type.capitalize(), replicate, file
                                ))
        else:
            for file in library_dict[library]:
                print('Hybrid test indices {0} in file {1}.'.format(
    ...
    for libr in libr_dict:
        if libr == 'H':
            param.mCP2_mismatch['25-32'] = mismatch_max(
                    'mismatch value for hybrid test constant part 2',
            ...
            param.mCP3_mismatch['25-32'] = mismatch_max(
                    'mismatch level for hybrid test constant part 3',
                    param.mCP3_length['25-32'] // 2
                    )
        else:
            if  libr_dict[libr]['mapping']:
                param.mCP2_mismatch[libr] = mismatch_max(
                        'mismatch value for mapping constant part 2 in library ',
                ...
                param.mCP3_mismatch[libr] = mismatch_max(
                        'mismatch value for mapping constant part 3 in library ',
                        param.mCP3_length[libr] // 2, libr
                        )                   

    param.mCP2_mismatch['29-36'] = mismatch_max(
            'mismatch level for ROI control constant part 2',
            len(param.mCP2['29-36']) // 2
            )
    param.mCP3_length['29-36'] = mismatch_max(
            'length of constant part 3 for ROI control',
            len(param.mCP3['29-36'])
            )
    param.mCP3_mismatch['29-36'] = mismatch_max(
            'mismatch level for ROI control constant part 3',
            param.mCP3_length['29-36'] // 2
            )
...
def mismatches_count(libr_dict, run_inf_text):
    print('Let\'s check acceptable mismatch levels.\n{0} mismatch in indices.'.format(
            str(param.index_mismatch_8bp)
            ))
    print('{0} mismatches in barcodes, {1} in ROIs.'.format(
    ...
    for lib in libr_dict:
        if lib == 'H':
            print('For chemera formation test {0} mismatches for mCP2;\n{1} mismatches for hybrid test constant part 3 {2} bp long.'.format(
    ...
    print('For mapping control {0} mismatches for mCP2;\n{1} mismatches for mCP3.'.format(
            str(param.mCP2_mismatch['29-36']),
            str(param.mCP3_mismatch['29-36'])
            ))
    ...
    lines_of_text = [
            '{0} mismatch in indices.\n'.format(str(param.index_mismatch_8bp)),
            '{0} mismatches in barcode, {1} in ROI.\n'.format(str(param.BC_mismatch), str(param.ROI_mismatch)),
            '{0} mismatches in normalization or expression constant part 1;\n{1} mismatches in normalization or expression constant part 2 {2} bp long.\n'.format(str(param.neCP1_mismatch), str(param.neCP2_mismatch), str(param.neCP2_length)),
            '{0} mismatches in mapping constant part 1\n'.format(str(param.mCP1_mismatch)),
            'For mapping control {0} mismatches for mapping constant part 2;\n{1} mismatches for mapping control constant part 3 {2} bp long.\n'.format(str(param.mCP2_mismatch['29-36']), str(param.mCP3_mismatch['29-36']), str(param.mCP3_length['29-36']))
            ]
    for lib in libr_dict:
        if lib == 'H':
            lines_of_text.append(
                    'For hybrid test {0} mismatches for mapping constant part 2;\n{1} mismatches for hybrid test constant part 3 {2} bp long.\n'.format(str(param.mCP2_mismatch['25-32']), str(param.mCP3_mismatch['25-32']), str(param.mCP3_length['25-32']))
                    )
        elif libr_dict[lib]['mapping']:
            lines_of_text.append(
                    'For library {0} {1} mismatches in mapping constant part 2;\n{2} mismatches for mapping constant part 3 {3} bp long.\n'.format(lib, str(param.mCP2_mismatch[lib]), str(param.mCP3_mismatch[lib]), str(param.mCP3_length[lib]))
                    )
...
def find_barcode(
    ...
    if score1 >= min_score1:
        for control_barcode in param.control_BCs[mode]:
            score_conbc, conbc_length, min_score_conbc = aligner(
                    rec.seq[(start + const1_length):],
                    param.control_BCs[mode][control_barcode], param.BC_mismatch
                    )
...
#Find barcodes and ROIs for hybrid test    
...
# Find barcodes and ROIs for mapping
def find_barcode_ROI(rec, ind, start, lib_dictionar, file):
    seq = rec.seq
    ...
    if score1 >= min_score1:
        for control_barcode in param.control_BCs['forward']:
            score_conbc, conbc_length, min_score_conbc = aligner(
                    seq[(start + const1_length):],
                    param.control_BCs['forward'][control_barcode],
                    param.BC_mismatch
                    )
            if score_conbc >= min_score_conbc:
                ...
                if score_cmut >= min_score_cmut:
                    if check_quality(
                        ...
                        score3, const3_length, min_score3 = aligner(
                                ...
                                param.mCP3['29-36'][:param.mCP3_length['29-36']],
                                param.mCP3_mismatch['29-36']
                                )
                        if score3 >= min_score3:
...
def fastq_parsing(ind_dictionary, lib_dictionary):
    ...
    for inp_file in ind_dictionary:
        undetermined_indices = 0
        low_quality_indices = 0
        total_read_count = 0
        ...
        for index in ind_dictionary[inp_file]:
            if index in param.mapping_indices:
                try:
        ...
        lines_of_text += [
                str(undetermined_indices) + ' reads without any index.\n',
                str(low_quality_indices) + ' reads with low quality index.\n'
                ]
...
def demultiplexing_or_not():
    ans = input('Do you need to perform demultiplexing of your fastq file(s)? Press Y or N: ').upper()
    if ans == 'Y' or ans == 'N':
        return ans
    else:
        print('You entered an incorrect value: %s. Try again.' % ans)
        ...
        for file in lib_dictionary[lib]['mapping']:
            ...
            if len(lib_dictionary[lib]['mapping'][file]) == 1:
                for index_list in lib_dictionary[lib]['mapping'][file]:
                    ...
                    for index in index_list:
                        for barcode in ind_dictionary[file][index]:
                            ...
                            elif barcode in param.control_BCs['forward']:
                                if barcode == 'bc_1' or barcode == 'bc_2':
                                    ROI = 'wt'
                                else:
                                    ROI = 'del_c'
                                if barcode in control_BCs_dict[file][', '.join(index_list)]:
                                    control_BCs_dict[file][', '.join(index_list)][barcode] += len(
                                            ind_dictionary[file][index][barcode][ROI]
                                            ) * 1000000 / ind_dictionary[file][index]['eff read count']
                                else:
                                    control_BCs_dict[file][', '.join(index_list)][barcode] = len(
                ...
                for i in range(len(lib_dictionary[lib]['mapping'][file])):
                    ...
                    control_BCs_dict[file][rep_name] = {}
                        ...
                        for barcode in ind_dictionary[file][index]:
                            ...
                            if len(barcode) > 15 and barcode not in [
                                ...
                                if barcode == 'bc_1' or barcode == 'bc_2':
                                    ...
                                    ROI = 'del_c'
                                if barcode in control_BCs_dict[file][rep_name]:
                                    control_BCs_dict[file][rep_name][barcode] += len(
                                            ind_dictionary[file][index][barcode][ROI]
                                            ) * 1000000 / ind_dictionary[file][index]['eff read count']
                                else:
                                    control_BCs_dict[file][rep_name][barcode] = len(
...
#This is temporal dictionary. Includes reads, found only in this replicate.
    ...
    if not gold_mapping:
        raise Exception('Error! There was not found any valid barcode-ROI pair for mapping of library %s'
                        % lib)

    return gold_mapping, mapping_count, control_BCs_dict
...
def form_gold_norm_or_expr(ind_dictionary, lib_dictionary, lib, rep_type):
    ...
    if len(lib_dictionary[lib][rep_type]) == 1:
        for file in lib_dictionary[lib][rep_type]:
            ...
            if len(lib_dictionary[lib][rep_type][file]) == 1:
                for index_list in lib_dictionary[lib][rep_type][file]:
                    ...
                    for control_barcode in control_BCs_dict[file][', '.join(index_list)]:
                        control_BCs_dict[file][', '.join(index_list)][control_barcode] = control_BCs_dict[file][', '.join(index_list)][control_barcode] / rep_eff_read_count * 1000000
                ...
                control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][0])] = {}
                for index in lib_dictionary[lib][rep_type][file][0]:
                    ...
                    for barcode in ind_dictionary[file][index]:
                        ...
                        if len(barcode) > 15 and barcode != 'low quality count':
                            ...
                            if barcode in control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][0])]:
                                control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][0])][barcode] += len(
                                        ind_dictionary[file][index][barcode]
                                        )
                            else:
                                control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][0])][barcode] = len(
                ...
                for control_barcode in control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][0])]:
                    control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][0])][control_barcode] = control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][0])][control_barcode] / first_rep_eff_read_count * 1000000
                for barcode in first_barcode_dict:
                ...
                for n in range(1, len(lib_dictionary[lib][rep_type][file])):
                    ...
                    control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][n])] = {}
                    rep_eff_read_count = 0
                    for ind in lib_dictionary[lib][rep_type][file][n]:
                        ...
                        for control_barcode in param.control_BCs['forward']:
                            if control_barcode in ind_dictionary[file][ind]:
                                if control_barcode in control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][n])]:
                                    control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][n])][control_barcode] += len(
                    ...
                    for control_barcode in control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][n])]:
                        control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][n])][control_barcode] = control_BCs_dict[file][', '.join(lib_dictionary[lib][rep_type][file][n])][control_barcode] / rep_eff_read_count * 1000000
        ...
        control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][0])] = {}
        for index in lib_dictionary[lib][rep_type][first_file][0]:
            ...
            #Check that it is not 'read count' or control barcode.
                if len(barcode) > 15 and barcode != 'low quality count':
                    ...
                    if barcode in control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][0])]:
                        control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][0])][barcode] += len(
                                ind_dictionary[first_file][index][barcode]
                                )
                    else:
                        control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][0])][barcode] = len(
        ...
        for control_barcode in control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][0])]:
            control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][0])][control_barcode] = control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][0])][control_barcode] / first_rep_eff_read_count * 1000000
        ...
        for n in range(1, len(lib_dictionary[lib][rep_type][first_file])):
            ...
            control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][n])] = {}
            rep_eff_read_count = 0
            for ind in lib_dictionary[lib][rep_type][first_file][n]:
                ...
                for control_barcode in param.control_BCs['forward']:
                    if control_barcode in ind_dictionary[first_file][ind]:
                        if control_barcode in control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][n])]:
                            control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][n])] += len(
                                    ind_dictionary[first_file][ind][control_barcode]
                                    )
            for control_barcode in control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][n])]:
                control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][n])] = control_BCs_dict[first_file][', '.join(lib_dictionary[lib][rep_type][first_file][n])] / rep_eff_read_count * 1000000
            ...
            if file != first_file:
                for index_list in lib_dictionary[lib][rep_type][file]:
                    ...
                    for ind in index_list:
                        ...
                        for control_barcode in param.control_BCs['forward']:
                            if control_barcode in ind_dictionary[file][ind]:
                                if control_barcode in control_BCs_dict[file][', '.join(index_list)]:
                                    control_BCs_dict[file][', '.join(index_list)][control_barcode] += len(
                                            ind_dictionary[file][ind][control_barcode]
                                            )
                                else:
                                    control_BCs_dict[file][', '.join(index_list)][control_barcode] = len(
    ...
    return gold_barcode_dict, barcode_count_dict, control_BCs_dict
...
def form_rep_expr(ind_dictionary, lib_dictionary, lib):
    ...
    return barcode_count_dict, control_BCs_dict, total_expression
...
def form_uniq_gold_dictionary(gold_dictionary):
    print(datetime.now())
    uniq_gold_dictionary = {}
    hash_length = int(param.BC_length // (param.BC_mismatch / 1.1 + 1))
...
#Find major ROI for each unique barcode.
            ...
            if gold_dictionary[lib]['barcodes-ROIs']:
                for uniq_barcode in uniq_gold_dictionary[lib]['barcodes']:
                    ...
                    if major_ROI in gold_dictionary[lib]['barcodes-ROIs'][uniq_barcode] and ROI_lists[0][1] >= total_ROI_count * param.major_ROI_portion:
            ...
            lines_of_text = [
                    ...
                    str(len(barcodes_to_remove)) + ' barcodes were removed from analysis due to high percent of chimeric molecules.\n'
                    ]
            print(lines_of_text)
            with open(run_info_text, 'a') as info_file:
                info_file.writelines(lines_of_text)
            ...
            for rep_type in rep_dictionary[lib]:
                ...
                if rep_type != 'mapping' and rep_dictionary[lib][rep_type]:
                    ...
                    if replicate_count >= 2:
                        ...
                        plt.savefig(
                                output_folder + '\\pair_plot_'+ rep_type + lib + '.pdf',
                                format='pdf', dpi=1000
                                )
                        plt.close()
                    ...
                    for file in rep_dictionary[lib][rep_type]:
                        ...
                        for replicate in rep_dictionary[lib][rep_type][file]:
                            ...
                            for barcode in uniq_gold_dictionary[lib]['barcodes-ROIs']:
                                ...
                                ROIs = major_ROI + '\n'
                                if barcode in rep_dictionary[lib][rep_type][file][replicate]:
                                    if major_ROI in rep_dictionary[lib][rep_type][file][replicate][barcode]:
                                        count = str(rep_dictionary[lib][rep_type][file][replicate][barcode][major_ROI]) + '\n'
                                        total_count += rep_dictionary[lib][rep_type][file][replicate][barcode][major_ROI]
                                    else:
                                        count = '0\n'
                                else:
                                    count = '0\n'
                                for ROI_variant in uniq_gold_dictionary[lib]['barcodes-ROIs'][barcode][barcode]:
                                    if ROI_variant != major_ROI:
                                        variants_of_barcode += '\n'
                                        ROIs += ROI_variant + '\n'
                                        if barcode in rep_dictionary[lib][rep_type][file][replicate]:
                                            if ROI_variant in rep_dictionary[lib][rep_type][file][replicate][barcode]:
                                                ...
                                                if ROI_variant not in uniq_gold_dictionary[lib]['barcodes-ROIs'][barcode]['major ROI'][major_ROI]:
                                        ...
                                        for mut_variant in uniq_gold_dictionary[lib]['barcodes-ROIs'][barcode][barc_variant]:
                                            ...
                                            if barc_variant in rep_dictionary[lib][rep_type][file][replicate]:
                                                if mut_variant in rep_dictionary[lib][rep_type][file][replicate][barc_variant]:
                                                    ...
                                                    if mut_variant != major_ROI and mut_variant not in uniq_gold_dictionary[lib]['barcodes-ROIs'][barcode]['major ROI'][major_ROI]:
                    ...
                    for barcode in uniq_gold_dictionary[lib]['barcodes-ROIs']:
                        ...
                        df_total_map_dict['Major ROI'].append(major_mut)
            ...
            if 'ROI' in new_columns_list:
                ...
                for index in lib_dict:
                    ...
                    if new_ROI in ROI_dictionary:
                        ROI_dictionary[new_ROI].append(lib_dict[index]['normalyzed to control'])
                    else:
                        ROI_dictionary[new_ROI] = [lib_dict[index]['normalyzed to control']]
                ...
                for m in range(1, 11):
                    lines_of_text.append(
                            '%s ROIs with %s barcodes for lib %s.\n' %(str(mut_barcode_count_list.count(m)), str(m), lib))
                    less_11 += mut_barcode_count_list.count(m)
                lines_of_text.append(
                        str(len(mut_barcode_count_list) - less_11) + ' ROIs with more than 10 barcodes for lib ' + lib + '.\n')
                ...
                sns.despine()
                plt.xlabel("Number of barcodes for each ROI for lib " + lib, fontsize=12)
                plt.savefig(
                        output_folder + '\\barc_per_ROI_' + lib + '.pdf',
                        format='pdf', dpi=100
                        )
                plt.close()
                ...
                plt.savefig(
                        output_folder + '\\ROI_with_different_barcodes_' + lib + '.pdf',
                        format='pdf', dpi=1000
                        )
                plt.close()
                ...
                if df_cont_dict['normalyzed expression mean'][0] > 0:
                    mut_ser = df_lib.groupby('ROI')['normalyzed to control'].mean()
                else:
                    mut_ser = df_lib.groupby('ROI')['normalyzed expression'].mean()
                mut_ser = mut_ser.sort_values(ascending=False)
                mut_table_path = output_folder + '\\' + lib + '_ROIs' + '.xlsx'
                ...
                sns.despine()
                plt.savefig(
                        output_folder + '\\kdeplot_' + lib + '.pdf',
                        format='pdf', dpi=1000
                        )
                if param.wt_ROI[lib] in ROI_dictionary:
                    ...
                    sns.despine()
                    plt.savefig(
                        output_folder + '\\kdeplot_and_wt_' + lib + '.pdf',
                        format='pdf', dpi=1000
                        )
                plt.close()
                ...
                sns.despine()
                plt.savefig(
                        output_folder + '\\log2_kdeplot_' + lib + '.pdf',
                        format='pdf', dpi=1000
                        )
                if param.wt_ROI[lib] in ROI_dictionary:
                    ...
                    sns.despine()
                    plt.savefig(
                        output_folder + '\\log2_kdeplot_and_wt_' + lib + '.pdf',
                        format='pdf', dpi=1000
                        )
                plt.close()
...
def make_graphs(uniq_gold_dictionary):
    for lib in uniq_gold_dictionary:
        if uniq_gold_dictionary[lib]['barcodes']:
            ...
            sns.despine()
            plt.xlabel("BC length for library " + lib, fontsize=12)
            plt.savefig(
                    output_folder + '\\BC_length_' + lib + '.pdf',
                    format='pdf', dpi=1000
                    )
            plt.close()
...
def make_pwm(mut_dict):
    ...
    for lib in mut_dict:
        ...
        for i in range(
            ...
            lines_of_text.append(
                    str(len(instances)) + ' ROIs ' + str(i) + ' nucleotides long for lib ' + lib + '.\n'
                    )
            print(str(len(instances)) + ' ROIs ' + str(i) + ' nucleotides long for lib ' + lib)
            if instances:
